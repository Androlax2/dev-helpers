...
export default class TextTruncate {
	...
	constructor($el, wordLimit, settings) {
		...
		if (this._hasShowMore()) {
			...
			if (!this.showMoreText
			    && this.settings.showMoreTextAttribute
			    !== '') throw new Error(`You need to set the attribute ${this.settings.showMoreTextAttribute} for the text.`);
			if (!this.showLessText
			    && this.settings.showLessTextAttribute
			    !== '') throw new Error(`You need to set the attribute ${this.settings.showLessTextAttribute} for the text.`);
		}
	...
	init() {
		...
		this.$el.setAttribute(
			'data-truncated-text',
			(
				content.slice(this.wordLimit, content.length)
			).join(' ')
		);
		...
		this.$el.dispatchEvent(new Event('init'));
	}
